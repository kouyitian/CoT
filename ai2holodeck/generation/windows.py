...
class WindowGenerator:
    ...
    def generate_windows(self, scene, additional_requirements_window):
        ...
        if "raw_window_plan" not in scene:
            ...
            history_constraint = constraint.format(history_chat["window"], deficiency)
            raw_window_plan = self.llm(history_constraint+window_prompt)
            # raw_window_plan = history_chat["window"].split("Agent:")[-1]
            history_chat["window"]="User:{} \n Agent:{}".format(history_constraint+window_prompt, raw_window_plan)
            history_chat["window"] = "{}".format(raw_window_plan)
