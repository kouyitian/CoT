...
class DoorGenerator:
    ...
    def generate_doors(self, scene, additional_requirements_door):
        ...
        if "raw_doorway_plan" not in scene:
            ...
            history_constraint = constraint.format(history_chat["door"], deficiency)
            raw_doorway_plan = self.llm(history_constraint+doorway_prompt)
            # raw_doorway_plan = history_chat["door"].split("Agent:")[-1]
            history_chat["door"]="User:{} \n Agent:{}".format(history_constraint+doorway_prompt, raw_doorway_plan)
            history_chat["door"]="{}".format(raw_doorway_plan)
