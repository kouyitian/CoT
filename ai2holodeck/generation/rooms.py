...
class FloorPlanGenerator:
    ...
    def generate_rooms(self, scene, additional_requirements="N/A", visualize=False):
        ...
        if "raw_floor_plan" not in scene:
            ...
            history_constraint = constraint.format(history_chat["room"], deficiency)
            raw_floor_plan = self.llm(history_constraint+floor_plan_prompt)
            # raw_floor_plan = history_chat["room"].split("Agent:")[-1]
            history_chat["room"]="User:{} \n Agent:{}".format(history_constraint+floor_plan_prompt, raw_floor_plan)
            history_chat["room"]="{}".format(raw_floor_plan)
