...
class ObjectSelector:
    ...
    def plan_room(self, args):
        ...
        from ai2holodeck.generation.prompts import history_chat, constraint, deficiency
        history_constraint = constraint.format(history_chat["plan1"], deficiency)
        output_1 = self.llm(history_constraint+prompt_1).lower()
        # output_1 = history_chat["plan1"].split("Agent:")[-1]
        plan_1 = self.extract_json(output_1)
        history_chat["plan1"]="User:{} \n Agent:{}".format(history_constraint+prompt_1, output_1)
        history_chat["plan1"] = "{}".format(output_1)
        ...
        if floor_capacity[1] / floor_capacity[0] >= required_floor_capacity_percentage or True:
            ...
            from ai2holodeck.generation.prompts import history_chat, constraint, deficiency
            history_constraint = constraint.format(history_chat["plan2"], deficiency)
            output_2 = self.llm(history_constraint+prompt_2).lower()
            # output_2 = history_chat["plan2"].split("Agent:")[-1]
            plan_2 = self.extract_json(output_2)
            history_chat["plan2"]="User:{} \n Agent:{}".format(history_constraint+prompt_2, output_2)
            history_chat["plan2"] = "{}".format(output_2)
    ...
    def get_floor_objects(
        ...
        for floor_object in floor_object_list:
            ...
            if variance_type == "same":
                ...
                selected_candidate = candidates[0] # new
