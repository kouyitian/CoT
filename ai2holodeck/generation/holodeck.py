...
class Holodeck:
    ...
    def generate_scene(
        ...
        prompts.history_chat = compress_json.load("history.json")
        ...
        import ai2holodeck.generation.prompts as prompts
        compress_json.dump(prompts.history_chat, os.path.join(save_dir, f"history.json"))
    ...
    def generate_improved_scene(
        ...
        # combine all history chat and new problem
        import ai2holodeck.generation.prompts as prompts
        prompts.constraint = prompts.new_constraint
        user_input = input("Please give feedback: ")
        if not user_input is None:
            print("Receive Feedbacks")
        prompts.deficiency = user_input
        ...
        compress_json.dump(
            scene,
            os.path.join(save_dir, f"{query_name}_revised.json"),
            json_kwargs=dict(indent=4),
        )
        compress_json.dump(prompts.history_chat, os.path.join(save_dir, f"history.json"))
        ...
        if generate_image:
            top_image = get_top_down_frame(scene, self.objaverse_asset_dir, 1024, 1024)
            top_image.show()
            top_image.save(os.path.join(save_dir, f"{query_name}_revised.png"))
