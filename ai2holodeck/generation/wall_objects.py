...
class WallObjectGenerator:
    ...
    def generate_wall_objects_per_room(self, args):
        ...
        if self.constraint_type == "llm" and use_constraint:
            ...
            history_constraint = constraint.format(history_chat["wall_obj"], deficiency)
            constraint_plan = self.llm(history_constraint+constraints_prompt)
            # constraint_plan = history_chat["wall_obj"].split("Agent:")[-1]
            history_chat["wall_obj"]="User:{} \n Agent:{}".format(history_constraint+constraints_prompt, constraint_plan)
            history_chat["wall_obj"] = "{}".format(constraint_plan)
