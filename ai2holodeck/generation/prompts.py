floor_plan_prompt = """You are an experienced room designer. Please assist me in crafting a floor plan. Each room is a rectangle. You need to define the four coordinates and specify an appropriate design scheme, including the room's color, material, and texture.
...
2. I need only the floor plan of one room.
...
4. Try to place more objects in the room to add richness.
...
# new_constraint = """
# <<{}>> \n
# The above content within the << >> is our previous conversation.
# Under your guidance, we have designed a room, but the following deficiencies still exist: {}.
# You must fully consider the content of these deficiencies.
# In response to these deficiencies, we will revise the entire design process by repeatedly asking the questions from our past discussions.
# If you believe that the repeated questions will not contribute to the improvement of the deficiencies, you can simply repeat the previous answers.
# Otherwise, please make different corrections to the previous answers.
# Our current question is as follows:
# \n
# """

# new_constraint = """
# <<{}>> \n
# The content within the << >> is our previous conversation.
# Under your guidance, we have designed a room, but the following deficiencies still exist: {}.
# We will revise the entire design process by repeatedly asking the questions from our past discussions.
# Our requirement is that the repeated questions must receive the exact same answers as the historical ones.
# Our current question is as follows:
# \n
# """
constraint = "{}{}"
# new_constraint = """
# <<{}>> \n
# The content within the << >> above is the answer you provided in our previous conversations.{}
# I will repeat the same question below, and please provide an exactly identical answer.
# Also, please do not retain the << >>.
...
new_constraint = """
{}\n
The content above is the answer you provided in our historical conversation.
With your guidance, we have designed a room.
However, it still has the following defects: [{}].
These defects are very important, and you must fully consider improvements to these defects.
We will now repeat the same questions below.
If you believe that the answer to the question will not help improve the defects, you must completely repeat the answer from the historical conversation.
If you think modifying your answer will have an impact on improving the defects, please follow the rules and provide a new answer.
The answer format must strictly follow the rules in the question and cannot be changed.
