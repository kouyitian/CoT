...
class WallGenerator:
    ...
    def get_wall_height(self, scene):
        ...
        if "wall_height" not in scene:
            ...
            history_constraint = constraint.format(history_chat["wall"], deficiency)
            wall_height_plan = self.llm(history_constraint+wall_height_prompt)
            # wall_height_plan = history_chat["wall"].split("Agent:")[-1]
            wall_height = wall_height_plan.split("\n")[0].strip()
            history_chat["wall"]="User:{} \n Agent:{}".format(history_constraint+wall_height_prompt, wall_height_plan)
            history_chat["wall"] = "{}".format(wall_height_plan)
